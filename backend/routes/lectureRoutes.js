...
router.get('/', async (req, res) => {
    try {
      ...
      const query = category ? { category } : {};
       ...
       // Create a cache key
      const cacheKey = `lectures:${category || 'all'}:page:${page}:limit:${limit}`;
      ...
      if (cached) {
        console.log('Lecture found in Redis cache');
      ...
      console.log('Lecture not found in cache, fetching from database...');
      ...
      console.log('Lecture found in database, caching in Redis...');
      // Cache the response in Redis (e.g., for 1 hour)
      await redisClient.setEx(cacheKey, 3600, JSON.stringify(response));
      console.log('Lecture cached in Redis');
  ...
  router.get('/:id', async (req, res) => {
    try {
      ...
      // Try to get from Redis cache first
      console.log('Checking Redis cache...');
      const cachedLecture = await redisClient.get(`lecture:${id}`);
      
      if (cachedLecture) {
        console.log('Lecture found in Redis cache');
        return res.json(JSON.parse(cachedLecture)); // Return cached
      }
      ...
      if (!lecture) {
        console.log('Lecture not found in database');
        return res.status(404).json({ message: 'Lecture not found' });
      }
      
      console.log('Lecture found in database, caching in Redis...');
      // Cache the lecture for future requests
      await redisClient.set(`lecture:${id}`, JSON.stringify(lecture));
      console.log('Lecture cached in Redis');
